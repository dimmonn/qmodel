...
echo "Obtaining docker-compose.yml from GitHub (without curl)..."
# clone shallowly, copy the compose file, then remove the temp clone
git clone --depth 1 https://github.com/<owner>/qmodel.git tmp-qmodel && cp tmp-qmodel/docker-compose.yml . && rm -rf tmp-qmodel
...
app.page_size=100
----
. Visit **Grafana Dashboard** → http://localhost:3001/d/adj5g9c/qmodel?orgId=1&from=2009-07-27T16:07:42.199Z&to=2026-04-24T07:03:58.581Z&timezone=browser (admin/admin).
...
* **Import data** by calling the API. See the API Reference section below for endpoint details.
...
The authoritative endpoint list is documented in `docs/API_REFERENCE.adoc` and is derived directly from `BaseGitQueryController`.

It covers the `/api/v1/metrics` base path, batch import endpoints, SZZ tracing routes, fork discovery, and commit/issue/pull ingestion flows.
...
A list of third-party components and their licenses is available in:

licenses/THIRD-PARTY.txt

The file is generated using the Maven license-maven-plugin.