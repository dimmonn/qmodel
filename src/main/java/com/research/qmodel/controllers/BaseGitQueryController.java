...
public class BaseGitQueryController extends GitMaintainable implements FileJsonReader {
    ...
    public BaseGitQueryController(
            BasicQueryService basicQueryService, DataPersistance dataPersistance, ProjectNameCache projectNameCache, ProjectIssueRepository projectIssueRepository, BasicBugFinder basicBugFinder, CommitRepository commitRepository) {
    ...
    public Object getActions(
            ...
            String repo) {
//https://api.github.com/repos/ansible/ansible/commits/76fb182b63a321aa513ee1c259eb39dd390e94c5/check-runs
        Action action =
                basicQueryService.retrieveMetrics(
                        "%s" + String.format("repos/%s/%s/commits/%s/check-runs", owner, repo),
                        new TypeReference<>() {
                        },
                        "?",
                        "&",
        ...
        return new ResponseEntity<>(
                dataPersistance.persistActions(
                        List.of(new Project(owner, repo)), Map.of(new Project(owner, repo), action)),
                HttpStatus.OK);
    }
    ...
    public Object baseQueryAg(
        ...
        projectNameCache.setProjectOwner(owner);
        projectNameCache.setProjectName(repo);
        final int FLUSH_EVERY = PAGE_SIZE;
    ...
    @GetMapping(value = "/repos/{owner}/{repo}/commits/actions")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<?> loadActionsPerCommit(
        ...
        List<Commit> allCommits = commitRepository.findAllCommitByProjectNameAndOwner(repo, owner).stream().filter(p -> p.getProjectName() != null).toList();
        for (Commit commit : allCommits) {
            String sha = commit.getSha();

            basicQueryService.retrieveMetricsWithStreaming(
                    "%s" + String.format("repos/%s/%s/commits/%s/check-runs", owner, repo, sha) + "%s",
                    new TypeReference<>() {
                    },
                    "?",
                    "&",
                    ...
                    (List<Map<String, Object>> batch) -> {
                        try {
                            dataPersistance.persistCommitActions(batch, sha);
        ...
        return ResponseEntity.ok("Commit Actions Import Completed");
    }
