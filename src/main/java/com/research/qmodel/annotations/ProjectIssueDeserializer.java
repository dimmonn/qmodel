...
//        Optional<ProjectIssue> foundIssue =
//            projectIssueRepository.findById(
//                new IssueID(owner, projectName, node.get("number").asLong()));
...
//        }
    ...
    if (node.get("labels") != null) {
      ...
      Set<String> rawLabels =
          labels.stream().map(e -> e.get("name").toString()).collect(Collectors.toSet());
  ...
  public static void main(String[] args){
    ...
    objectObjectHashMap.put("test1","val1");
    objectObjectHashMap.put("test2","val2");
    objectObjectHashMap.put("test3","val3");
    Set<Entry<String, String>> test1 = objectObjectHashMap.entrySet().stream()
        .filter(e -> !e.getKey().equals("test1")).collect(Collectors.toSet());
    System.out.println();
  }
}
